...
export async function CustomerListRequest(pageNo, perPage, searchKeyword) {
    try {
        ...
        let URL = "https://inventory-nu-six.vercel.app/CustomersList/"+pageNo+"/"+perPage+"/"+searchKeyword;
...
export async function CreateCustomerRequest(PostBody,ObjectID) {
    try {
        store.dispatch(ShowLoader())
        let URL = "https://inventory-nu-six.vercel.app/CreateCustomers"
        if(ObjectID!==0){
            URL = "https://inventory-nu-six.vercel.app/UpdateCustomers/"+ObjectID;
        }
...
export async function FillCustomerFormRequest(ObjectID) {
    try {
        store.dispatch(ShowLoader())
        let URL = "https://inventory-nu-six.vercel.app/CustomersDetailsByID/"+ObjectID;
        const result = await axios.get(URL,AxiosHeader)
        store.dispatch(HideLoader())
        if (result.status === 200 && result.data['status'] === "success") {
...
export async function DeleteCustomerRequest(ObjectID) {
    try {
        store.dispatch(ShowLoader())
        let URL = "https://inventory-nu-six.vercel.app/DeleteCustomer/"+ObjectID;
        const result = await axios.get(URL,AxiosHeader)
        store.dispatch(HideLoader())
