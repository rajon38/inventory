...
export async function PurchaseListRequest(pageNo, perPage, searchKeyword) {
    try {
        ...
        let URL = "https://inventory-nu-six.vercel.app/PurchasesList/"+pageNo+"/"+perPage+"/"+searchKeyword;
...
export async function SupplierDropDownRequest() {
    try {
        store.dispatch(ShowLoader());
        let URL = "https://inventory-nu-six.vercel.app/SuppliersDropDown";
        const result = await axios.get(URL,AxiosHeader)
        store.dispatch(HideLoader())
        if (result.status === 200 && result.data['status'] === "success") {
            if (result.data['data'].length > 0) {
...
export async function CreatePurchaseRequest(ParentBody,ChildsBody) {
    try {
        ...
        let URL = "https://inventory-nu-six.vercel.app/CreatePurchases"
